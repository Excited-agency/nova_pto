...
## Розгортання (25 серпня 2026)

Проєкт: `xlkqfqmppkgugagmofzy` («Nova», production).

### Порядок і результат

| Крок | Стан |
|---|---|
| Резервна копія балансів у `nova_backup.employee_balances_pre_accrual` | ✅ 170 рядків, 1025 днів |
| `supabase db push` — 9 міграцій | ✅ |
| `pg_cron` встановлено, завдання `nova-pto-daily-accruals` о 02:00 UTC | ✅ |
| Перерахунок балансів | ✅ 1025 → 3469 днів, 84 записи `recalculated` |
| Знімок назви категорії на всіх 32 заявках | ✅ |
| `apply_accruals(current_date)` вручну — мусить бути no-op | ✅ 3469 → 3469, журнал без змін |
| Edge-функції: `invite-employee`, `delete-workspace`, `slack-events` | ✅ |
| Радники безпеки — виправлено 2 власні попередження (`search_path`) | ✅ міграція `20260826130000` |
| Фронтенд — `800c322` + `ec4c7d7` | ✅ Production, живий бандл містить нові RPC |
| Кеш схеми PostgREST | ✅ усі RPC відповідають, внутрішні дають 42501 для anon |

### Перевірено на проді після розгортання

- `authenticated` **не має** INSERT/UPDATE на `time_off_requests` і записів у `employee_balances`, але читання працює
- усі 11 нових функцій на місці; `is_workspace_admin` перевіряє `status`
- `balance_adjustment_log.category_id` — nullable, FK `ON DELETE SET NULL`
- 4 нові тригери активні

### ⚠️ Помилка в порядку розгортання

Спершу задеплоїв базу, потім фронтенд — а живий фронтенд ще писав у таблиці
напряму (`time-off-request-service.ts` рядки 97, 155, 186 у старій версії).
У проміжку між `db push` і збіркою Vercel подання й відкликання заявок у
продакшені не працювали. Правильний порядок для такої зміни: **спершу
фронтенд** (він уже вміє і RPC, і старий шлях), і лише потім забирати права.

Причина знайдена: репозиторій переїхав у організацію, і Vercel втратив
зв'язок із GitHub — остання збірка була від 4 червня (`d74ed3f`), тобто
4 попередні коміти в прод не потрапляли. Після відновлення зв'язку Vercel
збирає лише на НОВИЙ пуш, тому знадобився порожній коміт (`ec4c7d7`).
Простій тривав близько 20 хвилин.

**Ще один урок:** мій скрипт-монітор хибно відзвітував «DEPLOYED».
`grep -c ... || echo 0` при нулі співпадінь віддає ДВА рядки ("0" від grep
і "0" від echo), тому порівняння `!= "0"` було істинним. Результат
перепроверено вручну — і лише тоді підтверджено.
...
from nova_backup.employee_balances_pre_accrual s
where s.id = b.id;
```
Копію не видаляти, поки цифри не підтверджені.
